...
S1V2lp <- AAS1V2l$arma[[1]]
S1V2lq <- AAS1V2l$arma[[2]]
S1V2ld <- AAS1V2l$arma[[5]]
ETSS1V1 <- ets(S1V1, model = 'ZZZ', ic = 'aicc')
ETSS1V2 <- ets(S1V2, model = 'ZZZ', ic = 'aicc')
ETSS1V2l <- ets(S1V2, model = 'ZZZ', ic = 'aicc', lambda = 'auto',
                biasadj = TRUE)
```

### Model Selection
Exponentially smoothed and ARIMA models were reviewed, with details in the
appendix. The selected model for V1 is an ARIMA(`r S1V1p`, `r S1V1d`, `r S1V1q`)
with drift and for V2 it is an ARIMA(`r S1V2lp`, `r S1V2ld`, `r S1V2lq`) with a
Box-Cox \(\lambda\) of `r AAS1V2l$lambda[[1]]`
...
autoplot(forecast(AAS1V1, h = 140)) + ylab("V1")
autoplot(forecast(AAS1V2l, h = 140)) + ylab("V2")
```
```{r S1Final, include=FALSE}
S1Final <- data.table(Var01 = forecast(AAS1V1, h = 140)$mean,
...
S2Final <- data.table(Var02 = rep(NA_real_, 140),
                      Var04 = rep(NA_real_, 140))
```
...
The cumulative error for our baseline model is 3700, which is meaningless in and
...
Exponential Smoothing models achieved an error of 3650, a slight improvement on
the baseline model.
...
The ARIMA model has an error of 3630, the lowest of the 3 models. All 3 models
...
satifactory level so no further differencing was applied.
...
plot(forecast(S4V2_boxed_outliers_autoarima, h = 140, biasadj = TRUE),
...
S5V2lp <- AAS5V2l$arma[[1]]
S5V2lq <- AAS5V2l$arma[[2]]
S5V2ld <- AAS5V2l$arma[[5]]
S5V3lp <- AAS5V3l$arma[[1]]
S5V3lq <- AAS5V3l$arma[[2]]
S5V3ld <- AAS5V3l$arma[[5]]
...
### Model Selection
Exponentially smoothed and ARIMA models were reviewed, with details in the
appendix. The selected model for V2 is an
ARIMA(`r S5V2lp`, `r S5V2ld`, `r S5V2lq`) model with
\(\lambda=\)`r AAS5V2l$lambda[[1]]`. The selected model for V3 is an
ARIMA(`r S5V3p`, `r S5V3d`, `r S5V3q`).
...
Both series have an extremely clear outlier that will need to be addressed. Both
series also appear to be almost 100% correlated. The data has a clear trend but
no visibly clear seasonal pattern.

### Missing Values

As per most other series, missing values were imputed linearly to maintain a
...
Before doing any outlier analysis it is very clear that there is a large outlier
that will need to be fixed in order to create stable predictions for this
timeseries.
...
We get a much better view of the time series as a whole, not obscured by the
huge outlier.
...
The baseline model has an error of 486. This is not meaningful in and of itself
but rather as a comparison to other models.
...
This model has an error of 481, slightly lower than the baseline.
...
The ARIMA model has an error of 482, slightly worse than the exponential
smoothing model. It also has a higher level of complexity so it makes sense to
select the simpler and more accurate model.
...
saveWorkbook(G1P1, file = './DT625_Summer2020_Project1_Group1.xlsx',
             overwrite = TRUE)
```
...
ARIMA(`r S1V1p`, `r S1V1d`, `r S1V1q`). However, looking at the characteristic roots
...
returned a `r ETSS1V1$method` model. The accuracy of the models are compared
...
be compared. For the ARIMA model it is `r AAS1V1$aicc` and for the ETS model, it
is `r ETSV1$aicc`. The choice is clear, that the ARIMA model is superior.

#### Selection
The ARIMA(`r S1V1p`, `r S1V1d`, `r S1V1q`) model will be used to forecast
Variable 01 for Series 01.
...
an ARIMA(`r S1V2p`, `r S1V2d`, `r S1V2q`) model. The autoregressive component
for V2 makes sense in the context of the partial autocorrelation plot above; the
fourth partial autocorrelation is larger in magnitude than all that follow. 
...
`r AAS1V2l$lambda[[1]]`) is once again an
ARIMA(`r S1V2lp`, `r S1V2ld`, `r S1V2lq`). It too cannot be directly compared to
the earlier two models, as it is based on different data. Its characteristic
roots are not much better than the original.
...
`r ETSS1V2$method` and `r ETSS1V2l$method` respectively.
...
knitr::kable(accS1V2, digits = 5L, format = 'pandoc')
```
...
#### Selection
The ARIMA(`r AAS1V2lp`, `r AAS1V2ld`, `r AAS1V2lq`) with lambda
`r AAS1V2l$lambda[[1]]` model will be used to forecast Variable 02 of Series 01.
...
residuals with no discernable pattern, constant variance, and a normal
...
under the transform, as opposed to making `r length(S5V2O$index)` indvidual
changes.
...
For V1, the best model using maximum likelihood is estimated as an
ARIMA(`r S5V2p`, `r S5V2d`, `r S5V2q`). Looking at the characteristic roots
is mainly reassuring. The AR roots are all well within the circle. The MA root
is close to the boundary, but is acceptable.
...
parsimonious ARIMA(`r S5V2lp`, `r S5V2ld`, `r S5V2lq`). This too exhibited safe
roots and acceptable residuals. What is of note is that despite a similar PACF
graph as to the untransformed series, the model with the lowest AICc has fewer
AR components than would have been expected.
...
most statistics, incluing the MAPE, point to the Box-Cox adjusted model. Thus,
rhe ARIMA(`r S5V2lp`, `r S5V2ld`, `r S5V2lq`) model with
\(\lambda=\)`r AAS5V2l$lambda[[1]]` will be used to forecast Series 05
Variable 02, with the bias adjustment to recover the mean and not the median.
...
untransformed V3 is an ARIMA(`r S5V3p`, `r S5V3d`, `r S5V3q`) model. Many of 
its characteristic roots are near the boundary, which is somewhat concerning.
```{r S5V3unitC1, echo=FALSE}
autoplot(AAS5V3) + ggtitle("Inverse Roots for Untransformed Series 5 V3")
```

The residuals look goood, though.
...
parsimonius ARIMA(`r S5V3lp`, `r S5V3ld`, `r S5V3lq`). The added MA root is well
within the circle, the others are similar to the untransformed model.

```{r S1V2l_unitC2, echo=FALSE}
autoplot(AAS5V3l)
```

Its residuals are eminentl acceptable as well.
```{r S1V2lresid, echo=FALSE, fig.width=10L, fig.height=8L}
...
#### Exponential Smoothing State Space
An ETS model with and without Box-Cox was also fit. These returned an
`r ETSV3$method` and `r ETSV3l$method` respectively.
...
fidelity, the ARIMA(`r S5V3p`, `r S5V3d`, `r S5V3q`) model will be used to
forecast Variable 03 for Series 05.
...
for(i in 100:(length(s6v5)-1)){
  ...
  error <- as.numeric(pred$mean) - s6v5[i+1]
  drift_sse_v1 <- drift_sse_v1 + error**2
}
drift_sse_v1
```
