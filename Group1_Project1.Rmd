...
appendix. The selected model for V1 is an `r arimaDisplay(AAS1V1)` with drift
and for V2 it is an `r arimaDisplay(AAS1V2l)` with a Box-Cox \(\lambda\) of
`r AAS1V2l$lambda[[1]]`
...
#### Selection
The `r arimaDisplay(AAS1V1)` model will be used to forecast Variable 01 for
Series 01.
...
`r AAS1V2l$lambda[[1]]`) is once again an `r arimaDisplay(AAS1V2l)`. It too
cannot be directly compared to the earlier two models, as it is based on
...
#### Selection
The `r arimaDisplay(AAS1V2l)` with \(\lambda=\)`r AAS1V2l$lambda[[1]]` model
will be used to forecast Variable 02 of Series 01.
