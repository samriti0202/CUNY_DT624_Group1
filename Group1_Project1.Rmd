...
be without drift. V1, in particular, looks very much like a random walk at first
...
ETSS5V2l <- ets(S5V2, model = 'ZZZ', ic = 'aicc', lambda = 'auto', biasadj = TRUE)
ETSS5V3 <- ets(S5V3, model = 'ZZZ', ic = 'aicc')
ETSS5V3l <- ets(S5V3, model = 'ZZZ', ic = 'aicc', lambda = 'auto', biasadj = TRUE)
```
...
The baseline model has an error of 486. This is not meaningful in and of itself but rather as a comparison to other models.
...
The ARIMA model has an error of 482, slightly worse than the exponential smoothing model. It also has a higher level of complexity so it makes sense to select the simpler and more accurate model.
