...
set of scatterplots, a helper function `scatterbox` will be defined.
```{r Q31aHist, fig.width = 9L, fig.height = 10L}
Glass$Type <- as.integer(levels(Glass$Type))[Glass$Type]
scatterbox <- function(df, box, useLast = TRUE) {
    par(mfrow = box)
...
categorical variable, that is less concerning.
...
to 0 and puts the variabiliyu on the same scale. This which will obviously not
be applied to the `Type` variable.
```{r Q31aZ, fig.width = 9L, fig.height = 10L}
scatterbox(scale(Glass), box = c(3, 3), useLast = FALSE)
```

Unfortuately, this did not help matters much.
...
```{r Q31aL, fig.width = 9L, fig.height = 8L}
scatterbox(log(Glass), box = c(3, 3), useLast = FALSE)
```
...
transforms which, given the parameter \(\lambda\), can include many types of 
transforms such as square, inverse, and power. An example of the transform with
\(\lambda = -1.2\) is shown below.
```{r Q31aBX, fig.width = 9L, fig.height = 10L}
bxcx <- function(x, l) {
    if (l == 0) {
        log(x)
    } else {
...
scatterbox(bxcx(Glass, -1.2), box = c(3, 3), useLast = FALSE)
```

There are packages in `R`, such as the `car` package, which will use maximum
likelihood to fit \(\lambda_i\) for each variable \(i\) in the dataset.
