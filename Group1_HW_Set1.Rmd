...
missing values and consider approaches to handling them. First, a breakout of
counts by class and then a list of missing data by variable for the classes with
missing data.
```{r QKJ32b3}
Sb[, .(Count = .N), keyby = Class]
Sb[Class %in% levels(Sb$Class)[c(1, 9, 10, 14, 16)],
   lapply(.SD, function(x) sum(is.na(x))), by = Class]
```
