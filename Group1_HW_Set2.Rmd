...
### Part (e)
The top 10 variabbles by importance are shown below:
```{r KJ63ea, echo=FALSE}
nI <- names(CMP)[-1]
t10 <- data.frame(nI, A$importance)[order(A$importance,
                                          decreasing = TRUE)[1:10], ]
t10N <- t10$nI
knitr::kable(t10, row.names = FALSE, format = 'pandoc', digits = 2)
```
...
`r t10N[1]`, `r t10N[7]`, and to a lesser extent `r t10N[5]` and demonstrate a
positive relation with yield. Conversely, `r t10N[2]` and `r t10N[3]` appear to
be negatively correlated. The remaining variables could almost be outlier
examples in Anscombe's famous quartet
...
The models from chapter 7[^kj75a] which will be investigated will include __ __ __

Similar to question 6.3, many versions of each model were fit. The one which is
displayed is usually a model whose tuning grid is focused on the area of best
behavior.[^kj75b] Seeds will be set before each call to ensure the repeatability
of the results.
...
[^kj75b]: This is a practical accomodation necesary to reduce the load on both
the machines and the times of the group members.

```{r KJ75preprocess, echo=FALSE}
set.seed(674835)
trainObs <- sample.int(n = dim(CMP)[[1]], size = dim(CMP)[[1]] %/% 2)
trainCMPX <- as.matrix(CMP[trainObs, -1])
trainCMPY <- unlist(CMP[trainObs, 1])
...
```{r KJ75aNN, include=FALSE}
fitControl <- trainControl(method = "repeatedcv", number = 11L, repeats = 5L,
                           allowParallel = TRUE)
nnGrid <- expand.grid(size = 1:3, decay =seq(0, 2e-5, 1e-6))
set.seed(12)
nnFit <- train(x = trainCMPX, y = trainCMPY, method = 'nnet',
...
```{r KJ75aSVM, include=FALSE}
rvmRGrid <- expand.grid(sigma = seq(5e-6, 8e-6, 5e-8))
set.seed(12)
