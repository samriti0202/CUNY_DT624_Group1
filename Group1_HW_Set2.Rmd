...
elastic_net <- train(train$x, train$y, method = 'glmnet', tunelength = 10, trControl = cv_control)

coef(elastic_net$finalModel, elastic_net$bestTune$.lambda)[,63]

elasticPred <- predict(elastic_net, newx = test$x)
...
nnetPred <- predict(nnet_model, newx = test_features)
...
marsPred <- predict(mars_model, newx = test_features)
...
svmPred <- predict(svm_model, newx = test_features)
...
knnPred <- predict(knn_model, newx = test_features)
